...
export const Heatmap: React.FC<HeatmapProps> = ({ 
  ...
  // Listen for storage changes from other components  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === EVENTS_STORAGE_KEY) {
        // If we're currently viewing events for a specific date, refresh those too
        if (selectedFutureEvent) {
          const updatedEvents = getEventsForDate(selectedFutureEvent.date);
          setSelectedDateEvents(updatedEvents);
          
          // If all events were removed, close the modal
  ...
  const handleAddEvent = (day: Date) => {
    ...
    // Check if it's a future date compared to the current real date
